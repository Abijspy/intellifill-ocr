...
public sealed record NativeOutputTable(string Label, IReadOnlyList<IReadOnlyList<string>> Rows);

public sealed record NativeRunValue(
...
public sealed class NativeExportService
{
    ...
    public void ExportCsv(IReadOnlyList<NativeOutputTable> tables, string path, string traceabilityCode)
    {
        var lines = new List<string> { $"Traceability,{EscapeCsv(traceabilityCode)}" };
        for (int tableIndex = 0; tableIndex < tables.Count; tableIndex++)
        {
            NativeOutputTable table = tables[tableIndex];
    ...
    public void ExportXlsx(IReadOnlyList<NativeOutputTable> tables, string path, string traceabilityCode)
    {
    ...
    public void ExportDocx(IReadOnlyList<NativeOutputTable> tables, string path, string traceabilityCode)
    {
        using FileStream file = File.Create(path);
        using var archive = new ZipArchive(file, ZipArchiveMode.Create);
        AddText(
        ...
        foreach (NativeOutputTable table in tables)
        {
    ...
    public void ExportPdf(IReadOnlyList<NativeOutputTable> tables, string path, string traceabilityCode)
    {
    ...
    private static string BuildWorkbook(IReadOnlyList<NativeOutputTable> tables)
    {
    ...
    private static string BuildWorksheet(NativeOutputTable table, string traceabilityCode)
    {
    ...
    private static XElement BuildWordTable(NativeOutputTable table)
    {
...
public sealed class NativeDatabaseService
{
    public void SaveRun(
        string databasePath,
        string traceabilityCode,
        ...
        IReadOnlyList<NativeRunValue> values,
        IReadOnlyList<string> mappingSummaries)
    {
        ...
        foreach (NativeRunValue value in values)
        {
