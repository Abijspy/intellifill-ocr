...
public sealed record NativeDocumentTable(string Label, IReadOnlyList<IReadOnlyList<string>> Rows)
{
    public int RowCount => Rows.Count;
    public int ColumnCount => Rows.Select(row => row.Count).DefaultIfEmpty(0).Max();
}

public sealed record NativeDocumentPreview(string Name, string Path, string ParsedText, IReadOnlyList<NativeDocumentTable> Tables);

public sealed class NativeTemplateLoader
{
    private static readonly XNamespace Spreadsheet = "http://schemas.openxmlformats.org/spreadsheetml/2006/main";
    private static readonly XNamespace Word = "http://schemas.openxmlformats.org/wordprocessingml/2006/main";

    public NativeDocumentPreview Load(string path)
    {
        ...
        return extension switch
        {
            ...
            ".txt" => LoadText(path),
            ".xlsx" => LoadXlsx(path),
            ".xls" => LoadPlaceholder(path, "Legacy .xls files are detected. Save as .xlsx for native WinUI table parsing."),
            ".docx" => LoadDocx(path),
            ".pdf" => LoadPlaceholder(path, "PDF preview is available as an imported document. Native PDF table parsing will be added in the WinUI-only engine."),
            ".png" or ".jpg" or ".jpeg" => LoadPlaceholder(path, "Image preview is available as an imported document. Native OCR will be added in the WinUI-only engine."),
            _ => LoadText(path)
        };
    }

    public NativeDocumentPreview LoadManyText(string path)
    {
        try
        {
            return Load(path);
        ...
        {
            return new NativeDocumentPreview(
                Path.GetFileName(path),
                path,
                $"Could not parse this file natively yet: {ex.Message}",
                Array.Empty<NativeDocumentTable>());
        }
    }

    private NativeDocumentPreview LoadCsv(string path)
    {
        ...
        return new NativeDocumentPreview(
            ...
            path,
            text,
            new[] { new NativeDocumentTable("Table 1", rows) });
    }

    private NativeDocumentPreview LoadText(string path)
    {
        ...
        return new NativeDocumentPreview(
            ...
            path,
            text,
            rows.Count > 0 ? new[] { new NativeDocumentTable("Text Lines", rows) } : Array.Empty<NativeDocumentTable>());
    }

    private NativeDocumentPreview LoadPlaceholder(string path, string message)
    {
        return new NativeDocumentPreview(
            Path.GetFileNameWithoutExtension(path),
            path,
            message,
            new[] { new NativeDocumentTable("Document", new[] { new[] { message } }) });
    }

    private NativeDocumentPreview LoadXlsx(string path)
    {
        using ZipArchive archive = ZipFile.OpenRead(path);
        List<string> sharedStrings = ReadSharedStrings(archive);
        var tables = new List<NativeDocumentTable>();
                     ...
                     entry.FullName.EndsWith(".xml", StringComparison.OrdinalIgnoreCase)))
        {
            ...
            if (rows.Count > 0)
            {
                tables.Add(new NativeDocumentTable($"Sheet {tables.Count + 1}", rows));
            }
        }
        ...
        return new NativeDocumentPreview(
            Path.GetFileNameWithoutExtension(path),
            path,
            string.Join(Environment.NewLine + Environment.NewLine, tables.Select(table => TableToText(table))),
            tables);
    }

    private NativeDocumentPreview LoadDocx(string path)
    {
        using ZipArchive archive = ZipFile.OpenRead(path);
        ...
        foreach (XElement tableElement in document.Descendants(Word + "tbl"))
        {
            ...
            if (rows.Count > 0)
            {
                tables.Add(new NativeDocumentTable($"Table {tables.Count + 1}", rows));
            }
        }
        ...
        {
            tables.Add(new NativeDocumentTable("Paragraphs", parsedText.Split(Environment.NewLine).Select(line => (IReadOnlyList<string>)new[] { line }).ToList()));
        }

        return new NativeDocumentPreview(Path.GetFileNameWithoutExtension(path), path, parsedText, tables);
    }
    ...
    private static string TableToText(NativeDocumentTable table)
    {
