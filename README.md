...
```text
IntelliFillOCR-2.2.3-win-x64.zip
IntelliFillOCR-Setup-2.2.3-win-x64.exe
```

and publishes both files to a GitHub release.

To publish version `2.2.3` manually:

1. Open the GitHub repository.
2. Go to **Actions**.
3. Select **CI/CD Release**.
4. Click **Run workflow**.
5. Keep version `2.2.3` and run it.
...
```text
installer\out\IntelliFillOCR-Setup-2.2.3-win-x64.exe
```
...
```text
msix\out\IntelliFillOCR_2.2.3.0_x64.msix
```
...
.\msix\install-msix.ps1 `
  -MsixPath .\msix\out\IntelliFillOCR_2.2.3.0_x64.msix `
